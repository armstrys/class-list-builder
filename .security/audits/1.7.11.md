...
assisted-by: claude-opus-4-7 via .claude/skills/security-audit-system
...
> claims in `SECURITY.md` (F-001 partial SRI, F-002 XLSX air-gap, F-003
...
(`.claude/skills/security-audit-system/`, `audits/`, `scripts/audit-hash.js`,
...
- Read every file under `src/` end-to-end with the prompt-injection-
  hardened security-audit-system skill.
- Ran `npm audit --json` and reviewed `metadata.vulnerabilities`.
- For each public claim in `SECURITY.md`, ran a targeted grep or
...
- **Description:** SECURITY.md (`SECURITY.md`, "Source Version
...
- **Location:** `SECURITY.md:200,212`; absent from
...
- **Location:** `SECURITY.md:157-161` vs.
...
and `SECURITY.md` for the sentinel patterns enumerated in
