  ...
  return (
    ...
    >
      ...
      <div className="class-col-cards">
        {students.length === 0 && (
          <div
            style={{ textAlign: 'center', padding: '20px 0', color: 'var(--text3)', fontSize: 12 }}
          >
            Drop students here
          </div>
        )}
      ...
      <div className="class-col-footer" style={fullscreen ? { display: 'none' } : {}}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
            {boolCounts.map(b => (
              <span
                key={b.label}
                className="badge"
                style={{ background: b.colors.bg, color: b.colors.fg }}
              >
                {b.label} {b.val}
              </span>
            ))}
        ...
        {flagCriteria.length > 0 && (
          <div style={{ marginTop: 5, fontSize: 11, color: 'var(--text2)' }}>
            Total Flags:{' '}
            <strong style={{ color: 'var(--text1)', fontFamily: "'DM Mono', monospace" }}>
              {classTotalFlagsCount}
            </strong>
          </div>
        )}
      </div>
