...
function ConstraintModal({ onClose }) {
  ...
  const tabs = [
    ...
    {
      id: 'outofclass',
      label: 'Keep Out of Class',
      count: keepOutOfClass.length,
      color: 'var(--warning)',
    },
  ];
  ...
  const title = (
    <span>
      Student Constraints{' '}
      {totalConstraints > 0 && (
        <span style={{ color: 'var(--text2)', fontWeight: 400 }}>({totalConstraints})</span>
      )}
    </span>
  );

  return (
    <Modal isOpen={true} onClose={onClose} title={title} size="lg" style={{ width: '90%' }}>
          ...
          {tabs.map(tab => (
            ...
            >
              ...
              {tab.count > 0 && (
                <span
                  className="badge"
                  style={{ background: tab.color, color: 'white', fontSize: 10 }}
                >
                  {tab.count}
                </span>
              )}
