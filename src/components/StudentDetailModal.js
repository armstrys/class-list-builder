function StudentDetailModal({
  ...
  const apartWithNames = keepApart.map(([id1, id2]) => ({
    ...
    otherName:
      studentById[id1 === student.id ? id2 : id1]?.name || (id1 === student.id ? id2 : id1),
  }));
  ...
  const outOfClassNames = keepOutOfClass.map(
    c => teachers[c.classIndex]?.name || `Class ${c.classIndex + 1}`
  );

  const hasConstraints =
    keepApart.length > 0 || keepTogether.length > 0 || keepOutOfClass.length > 0;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {numericCriteria.length > 0 && (
          <div>
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: `repeat(${Math.min(numericCriteria.length, 3)}, 1fr)`,
                gap: 10,
              }}
            >
        ...
        {inactiveFlags.length > 0 && (
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {inactiveFlags.map(c => (
                <span key={c.key} style={{ fontSize: 12, color: 'var(--text3)' }}>
                  {c.label}
                </span>
              ))}
            </div>
