...
function StatsStrip({ numClasses }) {
  ...
  const allItems = useMemo(() => {
    ...
    // Pre-calculate z-score parameters for total score calculation
    ...
    const studentScoreStrip = s =>
      zParamsStrip.reduce(
        (zs, { key, mean, stdDev }) => (stdDev > 0 ? zs + ((s[key] || 0) - mean) / stdDev : zs),
        0
      );
    ...
    if (flagCriteria.length > 0) {
      ...
      const cfSd = Math.sqrt(
        classMeanFlags.reduce((s, v) => s + (v - cfMean) ** 2, 0) / numClasses
      );
      compositeItems.push({
        label: 'Total Flags',
        vals: classMeanFlags,
        maxVal: Math.max(...classMeanFlags, 0.01),
        cv: cfMean > 0 ? cfSd / cfMean : 0,
      });
    }
    ...
    if (numericCriteria.length > 0) {
      ...
      const popVar =
        allTotalScores.reduce((s, v) => s + (v - popMean) ** 2, 0) / allTotalScores.length;
      ...
      compositeItems.push({
        label: 'Total Score',
        vals: classMeanScores.map(v => Math.max(0, (v - popMin) / popRange)),
        maxVal: 1,
        cv,
      });
    }
