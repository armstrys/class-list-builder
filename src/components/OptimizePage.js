...
function OptimizePage({ onBack }) {
  ...
  const runOptimize = useCallback(
    lockedAssignments => {
      ...
      setTimeout(() => {
        const lockedObj = {};
        lockedAssignments.forEach((classIdx, sid) => {
          lockedObj[sid] = classIdx;
        });
  ...
  const violations = useMemo(() => {
    ...
    return {
      ...
      totalViolations:
        apartViolations.length + togetherViolations.length + outOfClassViolations.length,
    };
  ...
  return (
    <div
      ...
      style={
        fullscreen ? { position: 'fixed', inset: 0, zIndex: 500, background: 'var(--bg)' } : {}
      }
    >
      {!fullscreen && (
        <div className="optimize-toolbar">
          ...
          <div className="lock-legend">
            ...
            <span>locked</span>
            <button className="btn btn-ghost btn-sm" onClick={handleLockAll}>
              Lock All
            </button>
            <button className="btn btn-ghost btn-sm" onClick={handleUnlockAll}>
              Unlock All
            </button>
          </div>
          ...
          {cost !== null && (
            <div className="score-badge">
              ...
              <span className="value" style={{ color: costColor }}>
                {cost.toFixed(4)}
              </span>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                >
                  <span
                    style={{
                      ...
                      height: 7,
                      borderRadius: '50%',
                      background: generateColor(c.key, i).dot,
                      flexShrink: 0,
                    }}
                  />
                  {c.label}
            ...
            >
              🔗 Constraints{' '}
              {keepApart.length + keepTogether.length + keepOutOfClass.length > 0 &&
                `(${keepApart.length + keepTogether.length + keepOutOfClass.length})`}
            </button>
            <button
              className="btn btn-secondary btn-sm"
              onClick={() =>
            ...
            visibleClasses.has(i) && (
              <ClassColumn
                ...
                onNameChange={(idx, val) =>
                  setTeachers(prev => prev.map((t, j) => (j === idx ? { ...t, name: val } : t)))
                }
      ...
      {showConstraints && <ConstraintModal onClose={() => setShowConstraints(false)} />}
